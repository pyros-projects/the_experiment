...
def load_datasets(page=1, per_page=10, search=None):
    ...
    return {"data": data[start_idx:end_idx], "total": total, "pages": (total + per_page - 1) // per_page}
...
def header_render(col, delimiter=None, subcolumns=None):
    ...
    return Th(
        Table(cls="w-full")(
            ...
            Tr(*[Th(f"Col {i + 1}", cls="text-center px-2 text-xs") for i in range(len(subcolumns))]),
        ),
        cls="p-2",
    )
...
def cell_render(col, val, delimiter=None):
    ...
    if not delimiter:
        return Td(cls="p-2")(Pre(val, cls="whitespace-pre-wrap font-mono text-sm"))

    # Only split into sub-columns if delimiter is specified
    values = split_data(val, delimiter)
    return Td(cls="p-2")(Table(cls="w-full")(Tr(*[Td(v, cls="text-center px-2 font-mono text-sm") for v in values])))
    ...
    def get(
        ...
        dataset_table = Div(cls="overflow-auto mt-4 rounded-md border border-border")(
            TableFromDicts(
                ...
                body_data=[{"Prompt": d["prompt"], "Completion": d["completion"].strip()} for d in result["data"]],
                body_cell_render=lambda col, val: cell_render(
                    col, val, delimiter=(prompt_delimiter if col == "Prompt" else completion_delimiter)
                ),
                header_cell_render=lambda col: header_render(
                    col,
                    delimiter=(prompt_delimiter if col == "Prompt" else completion_delimiter),
                    subcolumns=[f"Col {i + 1}" for i in range(prompt_cols if col == "Prompt" else completion_cols)]
                    if (prompt_delimiter if col == "Prompt" else completion_delimiter)
                    else None,
                ),
                sortable=False,
            )
        ...
        footer = DivFullySpaced(cls="mt-4 px-2 py-2")(
            Div(
                f"Showing {(page - 1) * per_page + 1} to {min(page * per_page, result['total'])} of {result['total']} entries",
                cls="flex-1 text-sm text-muted-foreground",
            ),
            Div(cls="flex flex-none items-center space-x-8")(
                DivCentered(f"Page {page} of {result['pages']}", cls="w-[100px] text-sm font-medium"),
                DivLAligned(
                    Button(
                        "<<",
        ...
        return Div(id="dataset-content")(table_controls, dataset_table, footer)

    return Sl_card(cls="rounded-lg shadow-lg ml-4 w-[100%]")(
        Div(H2("Dataset Explorer"), slot="header"),
        P("Browse and search through the training datasets", cls=TextFont.muted_sm),
        Div(id="dataset-content", hx_get="/datasets", hx_trigger="load"),
    )
