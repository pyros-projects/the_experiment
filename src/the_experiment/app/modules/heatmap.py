...
def get_layer_weights(model, layer_idx=0, component="mlp"):
    ...
    try:
        if component == "mlp":
            ...
            qkv_weight = (
                model.transformer.h[layer_idx].attn.c_attn.weight.detach().numpy()
            )
...
def WeightHeatmap(rt):
    ...
    if model is None:
        return Titled("Weight Display Test", P("Error: Model could not be loaded"))
        ...
        if component == "mlp":
            ...
            for name in ["query", "key", "value"]:
                plot_data = create_heatmap(
                    weights[name], f"Layer {layer} {name.title()} Weights"
                )
                ...
                sections.extend(
                    [
                        ...
                        P(
                            f"Weight matrix shape: {weights[name].shape[0]}×{weights[name].shape[1]}"
                        ),
