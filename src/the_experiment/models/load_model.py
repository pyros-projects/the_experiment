...
def check_out_folder(directory_path=DEFAULT_OUT) -> list[FolderContents]:
    try:
        ...
        for folder in folders:
            ...
            subfolder_exists = os.path.isdir(
                os.path.join(folder_path, "tiny-gpt2-causal")
            )
            rnn_file_exists = os.path.isfile(
                os.path.join(folder_path, "rnn_lm_final.pt")
            )
            cnn_file_exists = os.path.isfile(
                os.path.join(folder_path, "cnn_lm_final.pt")
            )
...
def load_rnn(folder):
    ...
    model = LSTMLanguageModel(
        vocab_size=50257, embed_dim=128, hidden_dim=128, num_layers=2
    )
...
def load_cnn(folder: str):
    ...
    model = CNNLanguageModel(
        vocab_size=50257, embed_dim=128, num_filters=128, kernel_size=3, seq_len=64
    )
