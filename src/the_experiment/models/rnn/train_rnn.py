...
def training_rnn(folder, callback=None):
    ...
    try:
        ...
        th = threading.Thread(target=background_thread, args=(callbacks,epochs,model,train_loader,device,vocab_size,criterion,optimizer,best_val_loss,out_dir,valid_loader,start_time), daemon=True)
        th.start()
...
def background_thread(callbacks, epochs, model, train_loader, device, vocab_size, 
                             criterion, optimizer, best_val_loss, out_dir, valid_loader, start_time):
    # Initialize monitor
    total_batches = len(train_loader) * epochs
    monitor = RNNTrainingMonitor(callbacks[0] if callbacks else None, total_batches, epochs)
