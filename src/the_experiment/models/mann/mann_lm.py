...
class MemoryUnit(nn.Module):
    ...
    def write(self, controller_state):
        ...
        erase_matrix = torch.bmm(write_weights.unsqueeze(-1), erase.unsqueeze(1))
        add_matrix = torch.bmm(write_weights.unsqueeze(-1), add.unsqueeze(1))
...
class MANNLanguageModel(nn.Module):
    ...
    def forward(self, input_ids, hidden=None):
        ...
        for t in range(seq_len):
            ...
            combined = torch.cat([controller_output[:, t], read_vector], dim=1)
