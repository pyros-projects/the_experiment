...
from causal_experiment.modules.app_frame import app_frame
...
app,rt = fast_app(hdrs=Theme.yellow.headers(highlightjs=True),live=True)


@rt('/')
def get(): 
    return Body(
            H1('Causal Experiment Kit',cls="ml-auto text-background bg-primary pl-3"), 
            P('we are hunting parrots tonight!',cls="ml-auto text-background bg-primary pl-3"),
            app_frame(rt)
        )
